...
export default function AffiliateTrainingAdminClient({ initialVideos }: { initialVideos: TrainingVideo[] }) {
  ...
  const openEdit = (v: TrainingVideo) => { setIsNew(false); setEditing({ ...v }); setError('') }
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch('/api/admin/affiliate-training', {
      ...
      if (isNew) setVideos(prev => [...prev, data.video])
      else setVideos(prev => prev.map(v => v.id === data.video.id ? data.video : v))
      setEditing(null)
    } catch (e: unknown) {
  ...
  const togglePublish = async (video: TrainingVideo) => {
    ...
    const data = await res.json()
    if (res.ok) setVideos(prev => prev.map(v => v.id === data.video.id ? data.video : v))
  }

  const deleteVideo = async (id: string) => {
    if (!confirm('Delete this video?')) return
    setDeleting(id)
    const res = await fetch(`/api/admin/affiliate-training?id=${id}`, { method: 'DELETE' })
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => setFilter(cat)}
              ...
              {cat.split('—')[0].trim()} ({videos.filter(v => v.category === cat).length})
            </button>
          ))}
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              ...
              <input value={editing.title ?? ''} onChange={e => setEditing(p => ({ ...p, title: e.target.value }))}
                placeholder="e.g. How to Pitch Program A" className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-xs font-medium text-gray-700 mb-1">Description</label>
              <textarea value={editing.description ?? ''} onChange={e => setEditing(p => ({ ...p, description: e.target.value }))}
                rows={2} placeholder="Brief description shown on the video card"
            ...
            <div className="sm:col-span-2">
              ...
              <input value={editing.embed_url ?? ''} onChange={e => setEditing(p => ({ ...p, embed_url: e.target.value }))}
                placeholder="YouTube /embed/ID · Loom /embed/ID · Guidde embed URL"
              ...
              <p className="text-xs text-gray-400 mt-1">Leave blank → shows as Coming Soon</p>
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Category</label>
              <select value={editing.category ?? 'Getting Started'} onChange={e => setEditing(p => ({ ...p, category: e.target.value }))}
            ...
            <div>
              ...
              <input value={editing.duration ?? ''} onChange={e => setEditing(p => ({ ...p, duration: e.target.value }))}
                placeholder="e.g. 4:20" className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 divide-y divide-gray-50 overflow-hidden">
              {catVideos.map(video => (
                <div key={video.id} className="flex items-center gap-3 px-4 py-3">
                  ...
                  {video.duration && <span className="text-xs text-gray-400 flex-shrink-0">{video.duration}</span>}
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button onClick={() => togglePublish(video)} title={video.is_published ? 'Unpublish' : 'Publish'}
