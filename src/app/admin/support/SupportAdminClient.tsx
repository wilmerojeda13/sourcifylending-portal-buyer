      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Reply to client messages — your reply is emailed and appears in their portal inbox</p>
          </div>
          <button onClick={refresh} disabled={refreshing}
            className="flex items-center gap-2 text-sm text-gray-600 border border-gray-200 px-3 py-2 rounded-xl hover:bg-gray-50 transition-colors disabled:opacity-50">
        ...
        <div className="flex items-center gap-2">
          ...
          {['all', 'open', 'replied', 'closed'].map(f => (
            <button key={f} onClick={() => setStatusFilter(f)}
              className={`text-xs font-semibold px-3 py-1.5 rounded-lg capitalize transition-colors ${statusFilter === f ? 'bg-green-600 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:border-green-400'}`}>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-3">
            {filtered.map(msg => {
              ...
              return (
                <div key={msg.id} className={`bg-white rounded-2xl border shadow-sm overflow-hidden transition-all ${msg.status === 'open' ? 'border-blue-200' : 'border-gray-100'}`}>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-0.5">
                          <span className="text-sm font-semibold text-gray-900">{msg.subject}</span>
                        ...
                        <p className="text-xs text-gray-500">
                          ...
                          <span className="text-gray-400"> · {msg.user_email} · {formatDate(msg.created_at)}</span>
                        </p>
                        {!isExpanded && (
                  ...
                  {isExpanded && (
                    <div className="px-5 pb-5 space-y-4 border-t border-gray-50">
                      ...
                      <div className="bg-gray-50 rounded-xl p-4 mt-4">
                        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Client Message</p>
                      ...
                      {msg.admin_reply && (
                        <div className="bg-green-50 border border-green-100 rounded-xl p-4">
                          <p className="text-xs font-semibold text-green-700 uppercase tracking-wide mb-2">Your Reply (sent to client)</p>
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          className="w-full px-4 py-3 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 resize-none transition-all"
                        />
