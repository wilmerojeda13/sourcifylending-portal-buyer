...
const STATUS_CONFIG: Record<CommissionStatus, { label: string; color: string }> = {
  ...
  reversed: { label: 'Reversed', color: 'bg-red-100 text-red-500' },
}
...
function TypeBadge({ type }: { type: CommissionType }) {
  return (
    <span className={`inline-flex items-center text-[11px] font-bold px-2 py-0.5 rounded-full capitalize ${TYPE_CONFIG[type] ?? 'bg-gray-100 text-gray-500'}`}>
      {type}
    </span>
  )
}
    ...
    return (
      <div className="pt-16 lg:pt-0 flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <p className="text-sm font-semibold text-gray-700">Failed to load commissions</p>
  ...
  return (
    <div className="space-y-6 pt-16 lg:pt-0">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Track your earned and paid commissions.</p>
      </div>

      {/* Summary stats */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'Total Earned', value: fmt(totalEarned), color: 'text-gray-900' },
          { label: 'Pending',      value: fmt(pending),     color: 'text-amber-600' },
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div className="px-5 py-3 border-b border-gray-100 flex items-center gap-1 overflow-x-auto">
          ...
          <span className="ml-auto text-xs text-gray-400 shrink-0">{total} records</span>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            <div className="w-12 h-12 bg-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-3">
              <ReceiptText size={22} className="text-gray-400" />
            </div>
            <p className="text-sm font-semibold text-gray-600">No commissions found</p>
                ...
                <tbody className="divide-y divide-gray-50">
                  {commissions.map((c) => (
                    <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3.5 text-xs text-gray-600 capitalize">
                        {c.program ? c.program.replace('_', ' ') : '—'}
                      </td>
                      <td className="px-4 py-3.5">
                        <TypeBadge type={c.commission_type} />
                      </td>
                      <td className="px-4 py-3.5 text-right text-xs text-gray-600">{fmt(c.gross_amount)}</td>
                      <td className="px-4 py-3.5 text-right text-xs text-gray-600">{fmtPct(c.commission_rate)}</td>
                      <td className="px-4 py-3.5 text-right text-sm font-bold text-gray-900">{fmt(c.commission_amount)}</td>
                      <td className="px-4 py-3.5">
                        <StatusBadge status={c.status} />
                      </td>
                      <td className="px-4 py-3.5 text-xs text-gray-500">{fmtDate(c.available_date)}</td>
                      <td className="px-4 py-3.5 text-xs text-gray-500">{fmtDate(c.paid_date)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <div key={c.id} className="px-4 py-4 space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-gray-400">{c.affiliate_referrals?.lead_email || ''}</p>
                    </div>
                    <StatusBadge status={c.status} />
                  </div>
                  <div className="flex items-center gap-2">
                    <TypeBadge type={c.commission_type} />
                    <span className="text-xs text-gray-400 capitalize">{c.program?.replace('_', ' ') || '—'}</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-500 text-xs">{fmt(c.gross_amount)} × {fmtPct(c.commission_rate)}</span>
                    <span className="font-bold text-gray-900">{fmt(c.commission_amount)}</span>
                  </div>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-2xl px-5 py-4">
        <div className="flex items-start gap-3">
          ...
          <p className="text-xs text-gray-500 leading-relaxed">
            Commissions become eligible after a <strong className="text-gray-700">7-day hold</strong> period.
            Minimum payout threshold: <strong className="text-gray-700">$100.00</strong>.
