...
const CATEGORY_CONFIG: Record<ResourceCategory, { label: string; color: string; bg: string }> = {
  ...
  marketing:       { label: 'Marketing',        color: 'text-blue-700',   bg: 'bg-blue-100' },
  program_summary: { label: 'Program Summary',  color: 'text-indigo-700', bg: 'bg-indigo-100' },
  sales_language:  { label: 'Sales Language',   color: 'text-green-700',  bg: 'bg-green-100' },
  how_it_works:    { label: 'How It Works',     color: 'text-purple-700', bg: 'bg-purple-100' },
}
...
function CategoryBadge({ category }: { category: ResourceCategory }) {
  const cfg = CATEGORY_CONFIG[category] ?? { label: category, color: 'text-gray-700', bg: 'bg-gray-100' }
  return (
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      <button
        ...
        className="w-full flex items-start gap-4 px-5 py-5 hover:bg-gray-50 transition-colors text-left"
      >
...
export default function AffiliateResourcesPage() {
  ...
  return (
    <div className="space-y-6 pt-16 lg:pt-0">
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Affiliate Resource Center</h1>
      ...
      <div className="bg-red-50 border border-red-200 rounded-2xl px-5 py-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-xs font-bold text-red-800 uppercase tracking-wide mb-0.5">Compliance Notice</p>
          ...
          <button
            ...
            className={`shrink-0 px-4 py-2 rounded-xl text-xs font-semibold transition-colors ${
              categoryFilter === value
                ...
                : 'bg-white border border-gray-200 text-gray-600 hover:border-indigo-200 hover:text-indigo-700'
            }`}
          >
            {label}
          </button>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm py-16 text-center">
          ...
          <p className="text-xs text-gray-400 mt-1">Check back soon — content is added regularly.</p>
        </div>
      ) : (
