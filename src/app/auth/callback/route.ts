...
export async function GET(request: NextRequest) {
  ...
  if (code) {
    ...
    if (!error) {
      ...
      if (user) {
        // ── Critical: duplicate-email guard (must await before redirect) ─────
        const { data: existingById } = await supabase
          .from('profiles').select('id').eq('id', user.id).maybeSingle()

        if (!existingById && user.email) {
          const svcCheck = createServerClient(
            process.env.NEXT_PUBLIC_SUPABASE_URL!,
            process.env.SUPABASE_SERVICE_ROLE_KEY!,
            { cookies: { getAll() { return [] }, setAll() {} } }
          )
          const { data: emailProfile } = await svcCheck
            .from('profiles').select('id').eq('email', user.email).neq('id', user.id).maybeSingle()
          if (emailProfile) {
        ...
        // ── Cookie cleanup (synchronous, no I/O) ─────────────────────────────
        ...
        // ── Fire-and-forget: all post-auth DB work ───────────────────────────
        ;(async () => {
          try {
                ...
                if (affiliate) {
                  ...
                  if (leadId) {
                    const { data: leadRecord } = await svc
                      .from('affiliate_leads').select('id, deal_type, email')
                      .eq('id', leadId).eq('affiliate_id', affiliate.id).maybeSingle()
                    if (leadRecord) { leadRecordId = leadRecord.id; leadDealType = leadRecord.deal_type || 'partner_assisted' }
                  }
                  ...
                  if (!existingRef && !isRetroactive) {
                    const { data: newReferral } = await svc.from('affiliate_referrals').insert({
                      ...
                      lead_email: user.email, referral_status: 'signed_up',
                    ...
                    if (leadRecordId) {
                      await svc.from('affiliate_leads').update({
                        ...
                        status: 'account_created', account_created_at: new Date().toISOString(),
                        acquisition_path: 'partner_assisted', onboarding_status: 'partner_closing',
                        partner_relationship_started_at: new Date().toISOString(), updated_at: new Date().toISOString(),
                      }).eq('id', leadRecordId)
                    }
                    if (isSelfReferral) {
                      await svc.from('affiliate_flags').insert({
                        affiliate_id: affiliate.id, flag_type: 'self_referral',
                        reason: `Affiliate ${affiliate.email} attempted to refer themselves`, status: 'pending',
                      })
                    }
                  }
                }
            ...
            if (!existingById) {
              const { error: profileErr } = await svc.from('profiles').insert({
                ...
                subscription_status: 'inactive', account_state: 'prospect',
                acquisition_path: refCode ? 'partner_assisted' : 'self_serve',
                progress_percentage: 0, nsf_flag: false,
                created_at: new Date().toISOString(), updated_at: new Date().toISOString(),
              })
              if (profileErr) {
                await recordSignupAutomationFailure({
                  userId: user.id, email: user.email ?? '',
                  stage: 'oauth_profile_upsert', source: 'google_oauth', errorMessage: profileErr.message,
                })
              }
              svc.from('activity_logs').insert({
                user_id: user.id, event_type: 'signup',
                event_data: { email: user.email, source: 'google_oauth' }, created_at: new Date().toISOString(),
              }).then(() => {})
            }
            ...
            if (refCode) {
              try {
                const { data: aff } = await svc
                  .from('affiliates').select('id, name').eq('referral_code', refCode.toUpperCase()).eq('status', 'active').maybeSingle()
                if (aff) {
                  const upd = await svc.from('profiles').update({
                    acquisition_path: 'partner_assisted', assigned_partner_affiliate_id: aff.id,
                    assigned_partner_name: aff.name, partner_relationship_started_at: new Date().toISOString(),
                    partner_onboarding_status: 'partner_closing', updated_at: new Date().toISOString(),
                  }).eq('id', user.id)
                  if (upd.error) {
                    await svc.from('profiles').update({
                      acquisition_path: 'partner_assisted', updated_at: new Date().toISOString(),
                    }).eq('id', user.id)
                  }
                }
              } catch { /* never break auth */ }
            ...
            // CRM SMS account link
            if (crmTextId) {
              await linkCrmSmsAccount(svc, {
                smsId: crmTextId, userId: user.id, profileId: user.id, email: user.email,
                createdBy: 'oauth_callback', metadata: { source: 'oauth_signup' },
              }).catch(() => {})
            }

            // CRM invite account link
            if (crmInviteId && user.email) {
              try {
                await linkCrmInviteAccount(svc, {
                  inviteId: crmInviteId, userId: user.id, profileId: user.id, email: user.email,
                  createdBy: 'auth_callback', metadata: { source: 'auth_callback' },
                })
              } catch { /* never break auth */ }
            }

            // Notifications + CRM lead + security events for new/fresh accounts
            if (isNewProfile || isFreshConfirmation) {
              ...
              logPortalEvent({
                userId: user.id, eventType: 'account_created', category: 'accounts', severity: 'success',
                title: 'New account created', message: user.email, metadata: { source, full_name: fullName },
              })
              try {
                const existingProfile = await svc
                  .from('profiles').select('business_name, suspicious_signup, signup_risk_score, suspicious_signup_reason')
                  .eq('id', user.id).maybeSingle()
                const crmResult = await ensureSignupCrmLead({
                  supabase: svc, userId: user.id, fullName, email: user.email ?? '',
                ...
                if (analyzerSessionId) {
                  await recordAnalyzerSessionEvent({
                    supabase: svc, sessionId: analyzerSessionId, eventType: 'account_created',
                    metadata: { user_id: user.id, email: user.email ?? '', lead_id: crmResult.leadId },
                  }).catch(() => {})
                }
                logPortalEvent({
                  userId: user.id, eventType: 'signup_crm_lead_created', category: 'leads',
                  severity: existingProfile.data?.suspicious_signup ? 'warning' : 'success',
                  title: 'Signup CRM lead created', message: user.email ?? '',
                  metadata: {
                    lead_id: crmResult.leadId, action: crmResult.action, source,
                ...
                await recordSignupAutomationFailure({
                  userId: user.id, email: user.email ?? '', stage: 'oauth_crm_lead_create',
                  source, errorMessage: getSignupAutomationErrorMessage(crmErr),
                })
                logPortalEvent({
                  userId: user.id, eventType: 'signup_crm_failed', category: 'leads', severity: 'critical',
                  title: 'Signup created without CRM lead', message: user.email ?? '',
                  metadata: { source, error: getSignupAutomationErrorMessage(crmErr) }, sendEmail: true,
                })
              }
              logSignupSecurityEvent({
                email: user.email ?? '', eventType: 'confirmed',
              ...
              if (contentAttribution?.pageId) {
                await recordContentEvent({
                  pageId: contentAttribution.pageId, eventType: 'signup',
                  relatedRecordId: user.id, metadata: { source, email: user.email },
                })
              }
            }
