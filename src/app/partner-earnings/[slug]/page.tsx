...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  const page = await getPublishedContentPage('partner-earnings', params.slug)
  return page ? buildPageMetadata(page) : {}
}

export default async function PartnerEarningsContentPage({ params }: { params: { slug: string } }) {
  const page = await getPublishedContentPage('partner-earnings', params.slug)
  if (!page) notFound()
